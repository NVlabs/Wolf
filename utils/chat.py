...
def process_image(image_path, img_processor, cross_img_processor, image):
    ...
    # all inputs will be entered into this pool
...
def process_image_new(image, img_processor, cross_img_processor):
    # all inputs will be entered into this pool
    if image is not None and isinstance(image, Image.Image):
        pil_img = image.convert('RGB')
        img_dict = img_processor(pil_img)
...
def chat(input_image, model, text_processor, img_processor,
    ...
    (torch_image, pil_img, cross_image) = process_image_new(input_image, img_processor, cross_img_processor)
